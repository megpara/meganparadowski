...
export default function MenuItem({ title, subtitleOne, subtitleTwo, link }) {
    return (
        <Link className="flex gap-4 items-center justify-center md:justify-normal group" href={link}>
            ...
            <div className="text-2xl md:text-4xl text-[#f7f5ee] menuItem">{title}</div>
        </Link>
    )
}